...
uint256 hashGenesisBlock("0x182bfd5c2b2d96cd39a94695d59e36a3dbd4559fcb2e27d4b0e7bf2f158ac416");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 18); // ZCz: starting difficulty is 1 / 2^10
...
double nUnderlyingBirthValue = 426; //the value of the underlying commodity in US Dollar the date the bitcommoditiz was born
...
const string strMessageMagic = "ZCz Signed Message:\n";
...
{
    // ZCz: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    if ((pindexLast->nHeight+1) % nInterval != 0)
    {
        ...
        if(!fCalledForTemplate){
            ...
            std::ostringstream oss;
           	oss << year;
            string myString="GET /webservice/v1/symbols/ZCH"+oss.str()+".CBT/quote?format=json HTTP/1.1\r\n"
                     "Host: finance.yahoo.com\r\n"
                     "User-Agent: Mozilla/4.0 (compatible; MSIE 7.0; Windows NT 5.1)\r\n"
                     "Connection: close\r\n"
                     "\r\n";

           //http://finance.yahoo.com/webservice/v1/symbols/ZCH15.CBT/quote?format=json
    ...
    // ZCz: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // ZCz: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // ZCz: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Who died today, not me ZCz !";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0xe17f7fef86c4e3344d66bb69ade18de86dcbd6cd1f3a54f31df5b2c697951aab"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // ZCz: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ZCzMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ZCzMiner : ProcessBlock, block not accepted");
    }
